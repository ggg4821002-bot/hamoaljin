...
async function verifyUser(ctx, next) {
    ...
    if (!isMember && !isDeveloper) {
        return ctx.replyWithPhoto(global.pp, {
            caption: "⚠️ *عذراً، يجب عليك الاشتراك في قنواتنا أولاً لاستخدام البوت!*",
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: [
                    [{ text: "📢 قناة التليجرام", url: global.CHANNEL_INVITE_LINK }],
                    [{ text: "👥 مجموعة التليجرام", url: global.GROUP_LINK }],
                    [{ text: "🔄 تحقق من الاشتراك", callback_data: "check_membership" }]
                ]
            }
        });
    } else {
...
async function startXeony() {
    ...
    try {
        ...
        if (XeonBotInc.callbackQuery.data === "check_membership") {
            ...
            if (isMember) {
                await XeonBotInc.answerCbQuery("✅ شكراً لاشتراكك! يمكنك الآن استخدام البوت.", { show_alert: true });
                await XeonBotInc.deleteMessage().catch(() => {});
            } else {
                await XeonBotInc.answerCbQuery("❌ عذراً، لم تشترك في جميع القنوات المطلوبة بعد.", { show_alert: true });
            }
    ...
    try {
        ...
        if (profilePhotos.photos.length > 0) {
            ...
            await XeonBotInc.replyWithPhoto(ownerPhoto, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'OWNER', url: "https://t.me/VikingTY" }, { text: 'CHANNEL', url: "https://t.me/VikingTY" }, { text: 'GROUP', url: "https://t.me/VikingTY2" }]
                    ]
                }
            });
        } else {
        ...
        await XeonBotInc.reply(lang.first_chat(BOT_NAME, user.full_name), {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: 'OWNER', url: "https://t.me/VikingTY" }, { text: 'GROUP', url: "https://t.me/VikingTY2" }]
                ]
            }
        });
    }
