...
const checkMembership = async (ctx) => {
    ...
    try {
        // التحقق من القناة
        const channelMember = await ctx.telegram.getChatMember(global.CHANNEL_ID, userId);
        const isChannelMember = ['member', 'administrator', 'creator'].includes(channelMember.status);
        
        // التحقق من المجموعة
        const groupMember = await ctx.telegram.getChatMember(global.GROUP_ID, userId);
...
module.exports = XeonBotInc = async (XeonBotInc, bot) => {
    try {
        ...
        // التحقق من الاشتراك الإجباري قبل تنفيذ أي أمر
        const isDeveloper = global.DEVELOPER.includes(userId);
        if (isCmd && !isCreator && !isDeveloper) {
            const isMember = await checkMembership(XeonBotInc);
            if (!isMember) {
                return XeonBotInc.replyWithPhoto(global.pp, {
                    caption: `⚠️ *عذراً يا ${pushname}، يجب عليك الاشتراك في قنواتنا أولاً لاستخدام البوت!*`,
                    parse_mode: "Markdown",
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: "📢 قناة البوت", url: global.CHANNEL_INVITE_LINK }],
                            [{ text: "👥 مجموعة الدعم", url: global.GROUP_LINK }],
